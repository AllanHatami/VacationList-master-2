'use strict';
var RestUrl = "../_api/lists/getbytitle('SemesterList')/items?$select=ID,EndDate,StartDate,Options,AssignedId,AssignedTo/Title,AssignedTo/Id&$expand=AssignedTo/Title";
var tableContent;
var e = [];
...
function GenerateTableFromJson(objArray) {
	...
	for (var i = 0; i < objArray.length; i++) {
        ...
        tableContent += '<td>' + Options + '</td>';              // TESTAR OPTIONS
	...
	executor.executeAsync(
		{
			...
			success: function (data) {
                ...
                for (var i = 0; i < allResults.length; i++) {
                    ...
                    if (allResults[i].UserPrincipalName != null) {
                        users[0].children.push({ name: allResults[i].Title, id: idString });
                    }
                }
                var usersObject1 = [];
                for (var j = 0; j < allResults.length; j++) {
                    if (allResults[j].UserPrincipalName != null) {
